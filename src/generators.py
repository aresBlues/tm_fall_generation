...
# Source: data/verified_public_figures.json (originally from input/verified_public_figures_extended.xlsx)
def _load_public_figures() -> list[tuple[str, str, str, str | None, str | None, str | None, str]]:
    """Load verified public figures from the JSON data file."""
    import json as _json
    from pathlib import Path
    data_path = Path(__file__).resolve().parent.parent / "data" / "verified_public_figures.json"
    with open(data_path, encoding="utf-8") as f:
        records = _json.load(f)
    return [
        (r["first_name"], r["last_name"], r["full_name"], r["date_of_birth"], r["city"], r["bekannt_durch"], r["industry"])
        for r in records
    ]


VERIFIED_PUBLIC_FIGURES: list[tuple[str, str, str, str | None, str | None, str | None, str]] = _load_public_figures()
